# 🎬 SYSTEM_DEMO.md — Review Auto Responder
...
обработано) с таймлайном каждого этапа. Метрики шагов — [🎛️ `OPERATOR_GUIDE.md` §7](OPERATOR_GUIDE.md).
...
Прямая ссылка на конкретный отзыв для разбора инцидента: полная трасса —
параметры запроса/исполнения, цепочка этапов с таймлайном. Метрики LLM-шага
(провайдер/модель, длительность, токены) — [🏗️ `ARCHITECTURE.md` §7](ARCHITECTURE.md).
...
снимок изменённого состояния. Фиксирует входы в `/admin`, смену конфигурации,
...
Прямая ссылка на конкретное событие: тип действия, исполнитель, параметры и снимок
состояния до/после. Секреты и полный текст промпта в журнал не пишутся.
