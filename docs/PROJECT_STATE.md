...
- Схлопывание execution-tracing в один POST (`create-with-steps`) при росте объёмов. Единственный оставшийся пункт PEcb09; ограничение актуально при высоких объёмах, на демо-трафике не проявляется (решение владельца 2026-08-30 — не брать в работу).
...
| 2026-08-30 | GigaChat CA-bundle на production | Долг PEcb09 закрыт: `certs/russian_trusted_ca_bundle.pem` в репозитории, compose-маунт + `GIGACHAT_CA_BUNDLE` по умолчанию; DEPLOYMENT_GUIDE (§3, §8.4) и `.env.example` актуализированы. Живой прод: worker пересоздан, E2E-отзыв обработан через GigaChat с верифицированным TLS (`using CA bundle`, CERT_NONE-warning исчез), ответ опубликован. Отпечатки и источник сертификатов — в гайде §8.4. Trace-батчинг (второй пункт PEcb09) — в «Возможном развитии» (решение владельца) |
