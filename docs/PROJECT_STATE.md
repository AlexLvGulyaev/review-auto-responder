...
10. ~~Сессионные ограничения сайта отзывов~~ — токенизированный демо-лимиттер с квотой на `POST /api/reviews` (модель `DemoSession`, `DemoLimiterService`, `POST /api/demo/start` + `GET /api/demo/status`, header `X-Demo-Token`, exempt воркера по `X-Worker-Token`); 3 уровня (sessions/IP/час, rate-limit, квота 5/сессию); UI-бейдж квоты.

### Возможное развитие (за границей v1.0)

- ~~CA-bundle для GigaChat на production вместо `ssl.CERT_NONE`~~ — ✅ реализовано 2026-08-30: `certs/russian_trusted_ca_bundle.pem` (Russian Trusted Root CA + Sub CA, источник gu-st.ru) в составе репозитория; compose монтирует его в `/certs/` и подставляет в `GIGACHAT_CA_BUNDLE` по умолчанию — проверка TLS включена из коробки. Верифицировано на живом демо: E2E-отзыв обработан, ответ GigaChat опубликован, в логах `GigaChat TLS: using CA bundle`, CERT_NONE-warning исчез.
- Схлопывание execution-tracing в один POST (`create-with-steps`) при росте объёмов. Единственный оставшийся пункт технического долга; ограничение актуально при высоких объёмах, на демо-трафике не проявляется (решение владельца 2026-08-30 — не брать в работу).
...
| 2026-08-30 | GigaChat CA-bundle на production | Долг закрыт: `certs/russian_trusted_ca_bundle.pem` в репозитории, compose-маунт + `GIGACHAT_CA_BUNDLE` по умолчанию; DEPLOYMENT_GUIDE (§3, §8.4) и `.env.example` актуализированы. Живой прод: worker пересоздан, E2E-отзыв обработан через GigaChat с верифицированным TLS (`using CA bundle`, CERT_NONE-warning исчез), ответ опубликован. Отпечатки и источник сертификатов — в гайде §8.4. Trace-батчинг (второй пункт долга) — в «Возможном развитии» (решение владельца) |
| 2026-09-03 | Обязательный технический долг закрыт (фиксация) | Решение владельца 03.09 (IMPLEMENTATION_PLAN AIP §3, аудит корпуса v1.22): обязательный технический долг проекта закрыт; оставшиеся пункты обязательным долгом не являются и ведутся как SHOULD/COULD-роадмап — «Возможное развитие»: batch execution tracing (актуально только при высоких объёмах) |
...
**Статус:** Портфельный актив. Реализован, Deployment Validation 18/18 PASS, живое демо. v1.5: демо-вход + сессионные лимиты
