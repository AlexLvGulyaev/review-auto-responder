...
<img src="docs/screenshots/RAR_portfolio_light.png" alt="Review Auto Responder — витрина кейса: публичный сайт отзывов с AI-ответами и операторская консоль (LIGHT)">

Уберите рутину из работы с отзывами: автономный конвейер 24/7 собирает новые записи с сайта, определяет тональность, генерирует уместный ответ нейросетью и уведомляет оператора в Telegram. Демонстрация универсального паттерна **«парсер + автономная AI-обработка»** с переключаемым LLM-провайдером.
...
**Статус:** Портфельный актив — живое демо: сайт отзывов с AI-ответами, операторская консоль, Telegram-уведомления
